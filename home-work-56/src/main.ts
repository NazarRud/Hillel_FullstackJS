...
function asyncOperationDemo(callback: (operation: string) => void): void {
  console.log('Перший виклик');

  const executeCallback = (operation: string): void => {
    try {
      callback(operation);
    } catch (error) {
      console.error(`Помилка в callback для ${operation}:`, error);
    }
  };

  process.nextTick(() => {
    console.log('Виконано nextTick');
    executeCallback('nextTick');
  });

  setImmediate(() => {
    console.log('Виконано setImmediate');
    executeCallback('setImmediate');
  });

  setTimeout(() => {
    console.log('Виконано setTimeout');
    executeCallback('setTimeout');
  }, 0);
