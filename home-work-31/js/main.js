...
async function getData(segment) {
    try {
        ...
        if (!response.ok) {
            console.error(`Помилка відповіді [GET]: ${response.status}`);
            return response.status;
        }

        return response.json();
    } catch (error) {
        console.error("Помилка запиту [GET]:", error);
...
async function postData(segment, data) {
    try {
        ...
        if (!response.ok) {
            console.error(`Помилка відповіді [POST]: ${response.status}`);
            return response.status;
        }

        return response.json();
    } catch (error) {
        console.error("Помилка запиту [POST]:", error);
        throw error;
    }
}
...
async function putData(id, data) {
    try {
        ...
        if (!response.ok) {
            console.error(`Помилка відповіді [PUT]: ${response.status}`);
            return response.status;
        }

        return response.json();
    } catch (error) {
        console.error("Помилка запиту [PUT]:", error);
        throw error;
    }
}
...
async function patchData(id, data) {
    try {
        ...
        if (!response.ok) {
            console.error(`Помилка відповіді [PATCH]: ${response.status}`);
            return response.status;
        }

        return response.json();
    } catch (error) {
        console.error("Помилка запиту [PATCH]:", error);
        throw error;
    }
}
...
async function deleteData(id) {
    try {
        ...
        if (!response.ok) {
            console.error(`Помилка видалення [id]: ${id} [DELETE]: ${response.status}`);
            return response.status;
        } 
        
        console.log(`Успішно видалено [id]: ${id}`);
        return true;
    } catch (error) {
        console.error("Помилка запиту [DELETE]:", error.message);
        return error.message;
    }
}
...
async function run() {
    try {
        //GET
        const getResult = await getData("/posts/1");
        console.log("GET =>");
        console.log(getResult);

        //POST
        const postResult = await postData("/posts", {
            userId: 1,
            title: "title test query Add",
            body: "body test query Add",
        });
        console.log("POST =>");
        console.log(postResult);

        //PUT
        const putResult = await putData(1, {
            id: 1,
            userId: 1,
            title: "title test query Update",
            body: "body test query Update",
        });
        console.log("PUT =>");
        console.log(putResult);

        //PATCH
        const patchResult = await patchData(1, {
            title: "title test query Update 2",
        });
        console.log("PATCH =>");
        console.log(patchResult);

        //DELETE
        const deleteResult = await deleteData(1);
        console.log("DELETE =>");
...
module.exports = { getData, postData, putData, patchData, deleteData };
