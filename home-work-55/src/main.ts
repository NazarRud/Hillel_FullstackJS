...
async function writeFileAsync(filename: string, content: string): Promise<void> {
  try {
    ...
    await fs.writeFile(path.join(filesDir, filename), content, 'utf-8');
    console.log('Файл успішно записано');
  } catch (error) {
...
async function readFileAsync(filename: string): Promise<void> { 
  try {
...
async function deleteFileAsync(filename: string): Promise<void> {
  try {
