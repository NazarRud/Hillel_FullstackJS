...
const theme = "color: green; font-weight: bold;";
...
class CalorieCalculator {
  constructor() {
    this.fruitsCalories = new Map();
  }

  addProduct(productName, calories) {
    this.fruitsCalories.set(productName, calories);
  }

  getProductCalories(productName) {
    if (this.fruitsCalories.has(productName)) {
      return this.fruitsCalories.get(productName);
    }
  ...
  removeProduct(productName) {
    this.fruitsCalories.delete(productName);
  }
}

// Демонстрація використання
console.log("%cЗавдання - #1", theme);
...
class UniqueUsernames {
  ...
  exists(username) {
    return this.uniqueUsers.has(username);
  }
