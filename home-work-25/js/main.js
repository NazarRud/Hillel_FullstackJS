...
function setUserInfoCookie(key, value) {
    ...
    const encodedVal = encodeURIComponent(`${key}=${value ?? ""}`);
    document.cookie = `userInfo=${encodedVal};expires=${date.toUTCString()};path=/`;

    console.log("Success set cookie userInfo to 10 sec.");
}
