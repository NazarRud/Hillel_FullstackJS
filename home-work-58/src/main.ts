...
function generatePasswordHash(password: string, salt: string, iterations: number = 10000, keylen: number = 64, digest: string = 'sha512'): string {
    try {
...
function verifyPassword(inputPassword: string, storedHash: string, salt: string, iterations: number = 10000, keylen: number = 64, digest: string = 'sha512'): boolean {
    try {
        const inputHash: string = crypto.pbkdf2Sync(inputPassword, salt, iterations, keylen, digest).toString('hex');
